        ...
        <div class="form-group">
            ...
            <div class="col-lg-8">
                <!--      <input type="text"  class="form-control" id="name" name="name" placeholder="请输入姓名">  -->
                     <input type="text" class="form-control" onblur="checkUserName(this.value)" id="username" name="username"  placeholder="请输入姓名">
                 </div>
    ...
    function checkUserName(username) {
        $.ajax({
            ...
            success:function(res) {
                if (res.isExist == "yes") {
                    //存在
                    $("#userSpan").html("抱歉，" + username + "已被注册，请更换！").css("color","red");
                } else if(res.isExist =="no") {
                    //不存在
                    $("#userSpan").html("恭喜，" + username + " 可以注册！").css("color","green");
                }<!-- $("#userSpan").html("恭喜，" + username + " 可以注册！").css("color","green"); -->
                else {
                    $("#userSpan").html("json解析失败")
                }
                alert(res);//[object Object]→dataType解析json字符串为json对象成功
                //{"isExist":"yes"}→dataType解析json字符串为json对象失败，还是json字符串
                alert(res.isExist);//yes or no是正确的，undefined是错误的
            }
        })
    }
</script>
